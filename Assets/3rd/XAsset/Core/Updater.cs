...
public class Updater : MonoBehaviour, IUpdater, INetworkMonitorListener
{
    ...
    private void Start()
    {
        ...
        LoadManager.updatePath = _savePath;
    }
    ...
    {
        OnMessage("正在初始化");
        var init = LoadManager.Initialize();
        yield return init;
        if(string.IsNullOrEmpty(init.error))
        {
            LoadManager.AddSearchPath("Assets/_Scenes");
            init.Release();
            OnProgress(0);
            OnMessage("加载游戏场景");
            var scene = LoadManager.LoadSceneAsync(gameScene, false);
            while(!scene.isDone)
            {
